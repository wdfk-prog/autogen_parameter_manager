...
- firmware-level object payload read/write uses dedicated object typed APIs by parameter number or external ID; generic `par_get_scalar()` / `par_set_scalar()` and scalar ID wrappers remain scalar-only
...
- `include/par.h` is the main public entry header. Application code should use `#include "par.h"`. Keep `parameters/include` on the compiler include path for public headers, and keep `parameters/src` on the include path for the generated definition and NVM default headers that `par_cfg.h` still includes (`def/par_def.h` and `nvm/par_nvm_cfg.h`). The packaged SConscript already adds both paths.
...
- The module separates **internal parameter enumeration** (`par_num_t`) from **external parameter IDs** (`id`). Generic scalar get/set entry points remain scalar-only; object payload read/write uses dedicated object typed APIs with both `par_num_t` and `*_by_id()` forms.
...
- `PAR_CFG_TABLE_ID_SCHEMA_VER` defaults in `include/par_cfg.h` and may be overridden in `port/par_cfg_port.h`; the integrator should bump it when intentionally changing the serialized table-ID schema.
