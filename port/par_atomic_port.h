 ...
 * @details Floating-point values are stored through the underlying atomic integer. type by preserving their bit representation.
 */
...
/**
 * @brief Convert a 32-bit floating-point value to its atomic storage representation. * This helper preserves the exact IEEE-754 bit pattern of a @c float by copying. its object representation into an integer-sized atomic storage type. * A normal C cast must not be used here. Casting from @c float to @c rt_atomic_t. performs a numeric conversion, not a bitwise reinterpretation. For example,. the bit pattern of @c 1.0f must be stored as @c 0x3f800000, while a numeric. cast would produce the integer value @c 1, which is not equivalent.
 * @note This function intentionally uses @c rt_memcpy to perform a bitwise copy. This avoids strict-aliasing violations and keeps the operation well-defined. under compiler optimization.
 * @note This design differs from implementations based on compiler built-ins such as. @c __atomic_store, where the compiler atomically copies bytes directly into a @c float object. Here, the floating-point value is first converted to the integer atomic storage format, and the actual atomic operation is then. performed by RT-Thread's integer atomic backend via @c rt_atomic_store().
 * @param value Floating-point value to convert.
 * @return Atomic integer value containing the exact bitwise representation of. @p value.
 */
...
/**
 * @brief Convert an atomic storage representation to a 32-bit floating-point value.
 *
 * This helper reconstructs a @c float from the raw bit pattern stored in an.
 * integer-sized atomic storage object.
 *
 * A normal C cast must not be used here. Casting from @c rt_atomic_t to @c float.
 * performs a numeric conversion, not a bitwise reinterpretation. For example,.
 * the stored bit pattern @c 0x3f800000 must be reconstructed as @c 1.0f, while.
 * a numeric cast would instead yield @c 1065353216.0f.
 *
 * @note
 * This function intentionally uses @c rt_memcpy to perform a bitwise copy.
 * This avoids strict-aliasing violations and keeps the operation well-defined.
 * under compiler optimization.
 *
 * @note
 * This design differs from implementations based on compiler built-ins such as.
 * @c __atomic_load, where the compiler atomically copies bytes directly from the
 * atomic object into a @c float variable. Here, RT-Thread first loads the raw.
 * integer atomic value via @c rt_atomic_load(), and this helper then converts.
 * that raw storage representation back into a floating-point value.
 *
 * @param value Atomic integer value containing a floating-point bit pattern.
 * @return point value reconstructed from @p value.
 */
